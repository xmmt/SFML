...
namespace
{
    struct VulkanLibraryWrapper
    {
        ...
        bool loadLibrary()
        {
            ...
            library = dlopen("libvulkan.1.dylib", RTLD_LAZY);
...
{
////////////////////////////////////////////////////////////
bool VulkanImplOSX::isAvailable(bool requireGraphics)
{
    ...
    if (!checked)
    {
        ...
        if (graphicsAvailable)
        {
            ...
            for (const VkExtensionProperties& properties : extensionProperties)
            {
                ...
                else if (!std::strcmp(properties.extensionName, VK_MVK_MACOS_SURFACE_EXTENSION_NAME))
                {
...
////////////////////////////////////////////////////////////
VulkanFunctionPointer VulkanImplOSX::getFunction(const char* name)
{
...
////////////////////////////////////////////////////////////
const std::vector<const char*>& VulkanImplOSX::getGraphicsRequiredInstanceExtensions()
{
...
////////////////////////////////////////////////////////////
bool VulkanImplOSX::createVulkanSurface(const VkInstance& instance, WindowHandle windowHandle, VkSurfaceKHR& surface, const VkAllocationCallbacks* allocator)
{
    if (!isAvailable())
        return false;

    WindowHandle viewHandle = getViewFromNSWindowPointer(windowHandle);
    ...
    vkCreateMacOSSurfaceMVK = reinterpret_cast<PFN_vkCreateMacOSSurfaceMVK>(wrapper.vkGetInstanceProcAddr(instance, "vkCreateMacOSSurfaceMVK"));

    if (!vkCreateMacOSSurfaceMVK) {
        throw std::runtime_error("Unabled to get pointer to function: vkCreateMacOSSurfaceMVK");
    }

    if (vkCreateMacOSSurfaceMVK(instance,&createInfo,allocator,&surface)!= VK_SUCCESS) {
